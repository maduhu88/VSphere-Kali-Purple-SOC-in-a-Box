...
2. Verify that there is ONLY ONE entry in the /etc/hosts file that maps the configured IP address to the configured hostname. There should be no other entries tied to the hostname. This will allow Kibana to specifically bind to that interface/IP address. ![](RackMultipart20231221-1-5l8g4e_html_bd192d06dbdbbf5e.png)
...
2. In a terminal session, paste the command and add " --force" on the end to auto-confirm installation of the Elastic Agent. Make sure there is a space between the port 8220 and the "--force" option. If you wish, you can change the IP address to your DETECT node's FQDN for the _fleet-server-es_ option.
